...
**Status 2026-09-15 (WP4b):** T057 red (2 new contract tests failed, host `swift test`), then green with T058–T060:
OnboardingKit 190/190. T058: the synchronized folder compiles the catalog into the appex (`de.lproj`/`en.lproj`
`Localizable.strings` confirmed in the built product), so **no pbxproj edit**. T060: `.onOpenURL` removed — no
`CFBundleURLTypes` and no associated-domains entitlement exist. T061: the warm-return case landed via a DEBUG seam
(`--uitest-pending-link-after-background`, `OwnFrame/UITestWarmReturnLinkSeam.swift`, red unit test first); the real
Safari → Share round trip was **not** driven from XCUITest and stays with T025 (`docs/hitl.md` §6). T062: build,
`ShareSheetIncomingUITests` 4/4 and the seam tests green on iOS 17.5; the EN/DE confirmation screenshots also wait for
that round trip. T063/T064 done in the same commit.

- [x] T057 [US2] Red: extend `Packages/OnboardingKit/Tests/OnboardingKitTests/ShareExtensionContractTests.swift` (text scan of the extension source) — the source MUST NOT contain `extensionContext?.open`, `extensionContext.open`, `openHost` or `immichslideshow://`; the confirmation string key ("Open OwnFrame to start") MUST appear in the extension source **and** in `OwnFrameShareExtension/Localizable.xcstrings` with a translated `de` entry. `@covers FR-210-31`. If the confirmation lands in its own file, widen the scan helper to every `.swift` file in `OwnFrameShareExtension/`, so the existing one-`.set(` / no-`URLSession` / no-`SecItem` guards (FR-210-13) keep covering all extension code. Red today: the open call exists, and so do the scheme and the missing catalog.
- [x] T058 [US2] **Claude inline**: create `OwnFrameShareExtension/Localizable.xcstrings` (EN source "Open OwnFrame to start" + a `Done` label if T059 uses a button; DE draft e.g. "Öffne OwnFrame, um zu starten") and wire it in `OwnFrame.xcodeproj/project.pbxproj`. The folder is a synchronized group (exception set excludes only `Info.plist`, ~:91-126), so first confirm the catalog is compiled into the appex's resources, and edit the pbxproj only where it is not. Check `knownRegions` includes `de` and the extension target's build settings (~:870) emit localized strings.
- [x] T059 [US2] Green, **Claude inline**: `OwnFrameShareExtension/ShareViewController.swift` — delete `hostScheme` (:24-26) and `openHost()` (:67-70). After the single App-Group write, present a minimal confirmation (UIKit label or a `UIHostingController` view; no network, no secret) with the localized text. Move `completeRequest` out of the `defer` in `handleShare()`, so the extension closes only after the confirmation is dismissed (a Done button, accessibility id `share.confirmation.done`, or a short auto-dismiss; record the choice here). With no URL, complete immediately. Rewrite the header comment (:5-9: "best-effort wake the host", "no UI"). Green T057. **Decided by Jan 2026-09-15: a Done button** (`share.confirmation.done`), no auto-dismiss — the person reads at their own pace and the close is UI-testable.
- [x] T060 [US2] **Claude inline**: fix the stale comments — `OwnFrame/Onboarding/IncomingLinkSheet.swift:5-6` (drop "the `immichslideshow://` hand-off"; the link is picked up when OwnFrame next becomes active) and `OwnFrame/OwnFrameApp.swift:652-661` (drop "when the Share Extension opens the host scheme"). Keep `.onOpenURL` (:663) unless it has no other use: check for a `CFBundleURLTypes` entry and an associated-domains entitlement. On 2026-09-14 grep found no other caller and no registered scheme, so remove it only after that check, and say so in the commit.
...
- [x] T063 Facts, **same commit as T059**: `product-facts.yaml` **SRC-07** → `implementation: verified`; remove `mismatch`/`candidate_issue`; add `FR-210-31` to `intent`; refresh `evidence` (new `ShareViewController.swift` lines, `OwnFrameShareExtension/Localizable.xcstrings`, `OwnFrameApp.swift` consume lines, `IncomingLinkSheet.swift`); add a limit that the extension tells the person to open OwnFrame. Run `.claude/scripts/check-facts.py`.
- [x] T064 Queue the new German string(s) for Jan's review in `docs/hitl.md`; commit with explicit paths; close #65 with the commit reference (`gh` account `kipp-ing`). T025 stays the device item and now also checks the confirmation.
