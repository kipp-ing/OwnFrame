...
- **FR-210-28**: In every surface that uses the picker (onboarding and Settings → Sources), adding an album MUST be select-then-confirm: tapping albums marks them and a pinned confirm commits, allowing one or more albums to be added in a single pass; tapping an album MUST NOT immediately add it and dismiss the picker. This applies to both the Immich album picker and the Photos (iCloud) album picker, and one pass may mix both tabs (Jan, 2026-09-14). *Amended 2026-09-15 (Jan):* the one surface exempt is the welcome screen's iCloud path (220, `photoLibrarySetup`), where tapping an album still saves it and starts the slideshow at once. That path adds a single first source and has no confirm step by design (220 clarification, lowest friction).
