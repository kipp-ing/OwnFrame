...
- [x] T001 Verify baseline green on `510-clock-overlay`: app target builds and full test
...
- [x] T002 Red tests in `Packages/ThemeKit/Tests/ThemeKitTests/ClockSettingsTests.swift`:
...
- [x] T003 Green: widen `Packages/ThemeKit/Sources/ThemeKit/ThemeSettings.swift`
...
- [x] T004 Red tests (same test file): `RandomPlacePicking` — no relocation before the
      ...
      SC-500-08).
- [x] T005 Green: add `Packages/ThemeKit/Sources/ThemeKit/ClockPlacement.swift` —
...
- [x] T009 [US4] Red-then-green: pill + analog styles in `ClockOverlayView.swift` with
...
- [x] T010 [US4] Red-then-green: Random wiring — photo-advance hook in `SlideshowView`
...
- [x] T011 [US4] Red XCUITest (settings scenario in `ClockOverlayUITests.swift`): open
...
- [x] T013 [P] [US4] Red tests in `Packages/HAControlKit/Tests/`: discovery publishes
...
- [x] T016 Screenshot gate: quickstart scenarios 3 + 5 captured on iOS 26.x iPad (glass)
...
- [x] T017 Full-suite gate (SC-510-01): entire XCUITest suite + all host suites green via
      ...
      this diff.)
- [x] T018 Docs + traceability: mark FR-510-01…06 / SC-510-01…02 rows done in
