...
//  OwnFrame
//
//  1100: the Supporter-Unlock gate on the HA/MQTT coordinator
...
/// - **No Supporter Unlock** → `.telemetryOnly`: a configured broker still connects and publishes
...
/// - **Supporter Unlock** → `.full`: read + control.
///
...
/// but the factory must still never *clear, migrate, or mask* stored configuration: buying the
/// Supporter Unlock later upgrades the same coordinator to `.full` with zero re-entry (FR-1100-14).
...
struct AutomationCoordinatorGate: Sendable {
    ...
    /// The tier the current entitlements allow: `.full` with the Supporter Unlock, else `.telemetryOnly`.
    func mode() -> HAControlCoordinator.Mode {
        entitlements().contains(.supporter) ? .full : .telemetryOnly
    }
